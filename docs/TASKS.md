...
| TASK-025 | P2 | Desktop / Accessibility | UI polish pass from the 2026-08-11 live audit — 6 of 8 fixed, 1 withdrawn, item 8 deferred | [docs/plans/2026-08-11-ui-polish.md](plans/2026-08-11-ui-polish.md) | Eight findings, all measured in the running app rather than reasoned about. Headline: the search/filter/sort toolbar shows no keyboard focus indicator on 3 of its 5 controls — the stylesheet defines the ring and inline `boxShadow: "none"` overrides it. Also 16×16 selection checkboxes, unnamed search inputs, required fields marked only by a glyph, an invalid Amount field with no associated message, and Analytics truncating the names it exists to show. Excludes contrast (TASK-022). **2026-08-13:** items 1-5 and 7 fixed and verified on real signed-in screens; item 6 withdrawn as unreproducible; item 8 (`h1`) deferred until TASK-022 lands, since it restyles headings app-wide. Three findings the plan never had were fixed too — 28 unnamed Settings row actions (one deletes), unnamed sign-in fields, sign-in hit targets. Harness checked in at `scripts/ui-audit.js` |
